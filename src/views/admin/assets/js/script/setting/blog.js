jQuery(document).ready(function ($) {
  ...
  jQuery.ajax({
        ...
        success: function (response) {
            ...
            data.forEach(function (item) {
                ...
                if (key === 'blog_banner_image') {
                    ...
                    $('#blog_banner_image').val(value);
                    $('#image_preview1').attr('src', value).show();
                } else  if (key === 'blog_banner_background_image') {
                    $('#blog_banner_background_image').val(value);
    ...
    const imageInput1 = $('#blog_banner_image1');
    imageInput1.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "blog_banner_image"
    ...
    function handleFileUpload(selectedFile, file_field) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (file_field === 'blog_banner_image') {
                    $('#image_preview1').attr('src', imagePath).show();z
                }else  {
