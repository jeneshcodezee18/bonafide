$("#addEvents").on("click", function () {
  window.location.href = BASE_URL + "admin_master/events/addEdit";
});
...
$("#events_form").on("submit", function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/events/add",
        data: {
        ...
        success: function (response) {
          ...
          if (response.err == 1) {
            ...
            window.location.href =
              BASE_URL + "admin_master/events/manage";
          }
        },
      });
...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin_master/events/list/${start}/${limit}`,
    data: {
      start: start,
      limit: limit,
    },
  ...
  if (list.length == 0) {
    ...
    for (let i = 0; i <= list?.length; i++) {
      if (i == list?.length) {
        ...
        htmlData += `<a href="` + BASE_URL + `admin_master/events/events_images?infographic_and_did_you_know_imagesid=${list[i].infographic_and_did_you_know_imagesid}" class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1" title="View">
  <span class="svg-icon svg-icon-3">
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"><path d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7Zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10Zm0-8a3 3 0 1 0 0 6 3 3 0 0 0 0-6Z" fill="black"/></svg>
  </span>
</a>`;
        htmlData += "</td>";
        htmlData += "</tr>";
...
$(document).ready(function () {
  ...
  if (didYouKnowId) {
    ...
    $.ajax({
      type: "POST",
      url: BASE_URL + "admin_master/events/view",
...
function remove(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/events/delete",
        data: {
          id: id,
        },
        success: function (response) {
