$("#addInfographicsImage").on("click", function () {
  window.location.href = BASE_URL + "admin_master/infographics/addEdit";
});
...
$("#infographics_form").on("submit", function (e) {
  e.preventDefault();
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/infographics/add",
        data: {
        ...
        success: function (response) {
          if (response.err == 1) {
            ...
            formElement.reset();
            window.location.href = BASE_URL + "admin_master/infographics/manage";
          }
        },
      });
    }
...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin_master/infographics/list/${start}/${limit}`,
    data: {
      start: start,
      limit: limit,
    },
...
$(document).ready(function () {
  ...
  const infographicsId = urlParams.get("infographic_and_did_you_know_imagesid");
  if (infographicsId) {
    // Call the view API to get job data
    $.ajax({
      type: "POST",
      url: BASE_URL + "admin_master/infographics/view",
      data: { id: infographicsId },
      success: function (response) {
        if (response.err == 0) {
...
function remove(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/infographics/delete",
        data: {
            id: id,
        },
        success: function (response) {
