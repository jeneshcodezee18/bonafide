$("#addEventsImages").on("click", function () {
  const parentId = new URLSearchParams(window.location.search).get("infographic_and_did_you_know_imagesid");
  window.location.href = BASE_URL + "admin_master/events/event_images/addEdit?infographic_and_did_you_know_imagesid=" + parentId;
  
});

const parentId =
  new URLSearchParams(window.location.search).get("infographic_and_did_you_know_imagesid") || "";
console.log("parentId:", parentId);
$("#events_images_form").on("submit", function (e) {
  e.preventDefault();
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/events/events_images/add",
        data: {
        ...
        success: function (response) {
          ...
          if (response.err == 1) {
            ...
            formElement.reset();
            window.location.href = BASE_URL + "admin_master/events/manage";
          }
        },
      });
...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin_master/events/events_images/list/${start}/${limit}`,
    data: {
      start: start,
      limit: limit,
      infographic_and_did_you_know_imagesid: parentId,
    },
    success: function (response) {
      if (response.err == 0) {
...
//   const imageId = urlParams.get("id"); // <-- use this for viewing existing image
//   const parentId = urlParams.get("infographic_and_did_you_know_imagesid"); // <-- still needed for saving

//   console.log("imageId (for edit):", imageId);
//   console.log("parentId (for context):", parentId);

//   if (imageId) {
//     // Call view API only when editing
//     $.ajax({
//       type: "POST",
//       url: BASE_URL + "admin_master/events/events_images/view",
//       data: { id: imageId },
//       success: function (response) {
//         if (response.err == 0) {
...
$(document).ready(function () {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const eventsImagesId = urlParams.get("infographic_and_did_you_know_imagesid");
  console.log("eventsImagesId (for edit):", eventsImagesId);
  if (eventsImagesId) {
    // Call the view API to get job data
    $.ajax({
      type: "POST",
      url: BASE_URL + "admin_master/events/events_images/view",
      data: { id: eventsImagesId },
      success: function (response) {
        if (response.err == 0) {
          var data = response.data;
          $("#image_preview").attr("src", data.image).show();
...
function remove(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/events/events_images/delete",
        data: {
          id: id,
        },
        success: function (response) {
