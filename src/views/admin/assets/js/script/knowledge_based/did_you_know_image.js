      ...
      jQuery.ajax({
        ...
        success: function (response) {
          ...
          if (response.err == 1) {
            ...
            formElement.reset(); 
            window.location.href = BASE_URL + "admin_master/did_you_know/manage"
          }
        },
      });
  ...
  if (list.length == 0) {
    ...
    for (let i = 0; i <= list?.length; i++) {
      if (i == list?.length) {
        ...
        htmlData += '<td><img src="' + (list[i].image) + '" alt="Image" width="50" height="50"></td>';        htmlData += `<td>${list[i].report_title}</td>`;
        htmlData +=
