...
$("#partner_form").on("submit", function (e) {
  ...
  var formElement = this;
    ...
    const requiredFields = [
        ...
        { id: 'website_url', label: 'URL' },
        { id: 'brief_introduction', label: 'Brief Information' },
    ...
    // Image validation
    if (!$('#image').val() && !$('#image1')[0].files.length) {
        $('#image1').closest('.image-input').addClass('is-invalid');
        errors.push('Image is required');
    }

    // URL validation
    const urlRegex = /^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w .-]*)*\/?$/;
    if ($('#website_url').val() && !urlRegex.test($('#website_url').val())) {
        $('#website_url').addClass('is-invalid');
        errors.push('Please enter a valid URL');
    }

    // CKEditor content validation
    const editorContent = editor.getData().trim();
    if (!editorContent) {
        $('#kt_long_text').addClass('is-invalid');
        errors.push('Long Text content is required');
    }
  ...
  if (list.length == 0) {
    ...
    for (let i = 0; i <= list?.length; i++) {
      if (i == list?.length) {
        ...
        htmlData += `<td>
          <a href="${BASE_URL}admin_master/partner_list/addEdit?partner_id=${list[i].partner_id}" class="text-gray-800 text-hover-primary fw-bold">
          ${list[i].first_name}
          </a>
        </td>`;
