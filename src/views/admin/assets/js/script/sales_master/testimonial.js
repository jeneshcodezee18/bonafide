$("#addTestimonial").on("click", function () {
  window.location.href = BASE_URL + "admin_master/testimonial/addEdit";
});
...
$("#testimonial_form").on("submit", function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/testimonial/add",
        data: {
          testimonialid: $("#Id").val(),
          title: $("#title").val(),
          name: $("#name").val(),
          post: $("#post").val(),
        ...
        success: function (response) {
          if (response.err == 1) {
            ...
            formElement.reset();
            window.location.href = BASE_URL + "admin_master/testimonial/manage";
          }
        },
      });
    }
...
function remove(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/testimonial/delete",
        data: { id: id },
        success: function (response) {
          if (response.err == 1) {
            Swal.fire({
...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin_master/testimonial/list/${start}/${limit}`,
...
$(document).ready(function () {
  // Get testimonialid from query string
  const urlParams = new URLSearchParams(window.location.search);
  const testimonialId = urlParams.get("testimonialid");
  if (testimonialId) {
    $.ajax({
      type: "POST",
      url: BASE_URL + "admin_master/testimonial/view",
      data: { testimonialid: testimonialId },
      success: function (response) {
        if (response.err == 0) {
          var data = response.data;
          $("#Id").val(data.testimonialid || "");
