$("#addMember").on("click", function () {
  window.location.href = BASE_URL + "admin_master/home_page/network_of_field_surveyors_list_addEdit";
});
...
$("#member_form").on("submit", function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/member/add",
        data: {
          memberid: $("#Id").val(),
          f_name: $("#f_name").val(),
          l_name: $("#l_name").val(),
...
function remove(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin_master/member/delete",
        data: {
          id: id,
        },
        success: function (response) {
...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin_master/member/list/${start}/${limit}`,
    data: {
      start: start,
      limit: limit,
    },
...
$(document).ready(function () {
  ...
  if (memberId) {
    ...
    $.ajax({
      type: "POST",
      url: BASE_URL + "admin_master/member/view",
      ...
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#calendly_url").val(data.calendly_url || "");
