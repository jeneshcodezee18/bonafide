jQuery(document).ready(function ($) {
  ...
  jQuery.ajax({
    ...
    const imageInput1 = $('#contact_image1');
    imageInput1.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "contact_image"
            handleFileUpload(selectedFile, file_field);
        }
    });
    const imageInput2 = $('#footer_logo1');
    imageInput2.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "footer_logo"
            handleFileUpload(selectedFile, file_field);
        }
    });
    ...
    function handleFileUpload(selectedFile, file_field) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (file_field === 'contact_image') {
                    $('#image_preview1').attr('src', imagePath).show();
                } else if(file_field === 'footer_logo') {
    ...
    $('#footer_details_form').on('submit', function (e) {
        e.preventDefault();
