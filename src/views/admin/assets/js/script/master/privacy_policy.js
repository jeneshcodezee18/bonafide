...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin/master/privacy_policy/list/${start}/${limit}`,
    headers: {
...
function html(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    htmlData += `<td>${list[i].privacy_policy_heading}</td>`; 
    htmlData += `<td>${list[i].privacy_policy_detail}</td>`; 
...
function add_record() {
  ...
  $("#edit_privacy_policy").trigger("reset");
  $(".popup-title").html("<h2>Add Privacy Policy</h2>");
}

const quill1 = new Quill("#kt_privacy_policy_detail", {
  theme: "snow",
});


$("#edit_privacy_policy").on("submit", function (e) {
  e.preventDefault();
  ...
  Swal.fire({
    ...
    customClass: {
      ...
      if (result.isConfirmed) {
          ...
          jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin/master/privacy_policy/add",
        headers: {
          authorization: token ? `Bearer ${token}` : "",
        },
        data: {
          id: $("#Id").val(),
          privacy_policy_heading: $("#privacy_policy_heading").val(),
          privacy_policy_detail: editorContentHTML1,
...
function view(id) {
  ...
  jQuery.ajax({
    ...
    url: BASE_URL + "admin/master/privacy_policy/view",
    data: {
      id: id,
    },
    success: function (response) {
      if (response.err == 0) {
        ...
        $("#Id").val(data._id);
        $("#privacy_policy_heading").val(data.privacy_policy_heading);
        const delta = quill1.clipboard.convert(data.privacy_policy_detail);
        quill1.setContents(delta);
        $(".popup-title").html("<h2>Edit Privacy Policy</h2>");
        $("#edit-modal").modal("show");
      }
    },
  });
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      jQuery.ajax({
        ...
        url: BASE_URL + "admin/master/privacy_policy/delete",
        data: {
          id: id,
        },
        success: function (response) {
