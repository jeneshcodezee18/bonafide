...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin/master/testimonial/list/${start}/${limit}`,
    headers: {
...
function html(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    htmlData += '<td><img src="' + (list[i].testimonial_thumbnail_image) + '" alt="Image" width="50" height="50"></td>';
    htmlData += `<td>${list[i].user_name}</td>`;
    htmlData += `<td>${list[i].city}</td>`;
...
function add_record() {
  ...
  $("#edit_testimonial").trigger("reset");
  $(".popup-title").html("<h2>Add Testimonial</h2>");
}

const imageInput1 = $('#testimonial_thumbnail_image1');
imageInput1.on('change', function (e) {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
        const file_field = "testimonial_thumbnail_image"
        handleFileUpload(selectedFile, file_field);
    }
});
const videoInput2 = $('#testimonial_video1');
videoInput2.on('change', function (e) {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
        const file_field = "testimonial_video"
        handleFileUpload(selectedFile, file_field);
    }
});
function handleFileUpload(selectedFile, file_field) {
  ...
  $.ajax({
      ...
      success: function (response) {
          ...
          if (file_field === 'testimonial_thumbnail_image') {
              $('#image_preview1').attr('src', imagePath).show();
          }else if (file_field === 'testimonial_video') {
...
const quill1 = new Quill("#kt_testimonial_detail", {
  theme: "snow",
});
  ...
  Swal.fire({
    ...
    customClass: {
      ...
      if (result.isConfirmed) {
          ...
          jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin/master/testimonial/add",
        headers: {
          authorization: token ? `Bearer ${token}` : "",
        },
        data: {
          ...
          city: $("#city").val(),
          testimonial_detail: editorContentHTML1,
          testimonial_thumbnail_image: $("#testimonial_thumbnail_image").val(),
          testimonial_video: $("#testimonial_video").val(),
...
function view(id) {
  ...
  jQuery.ajax({
    ...
    url: BASE_URL + "admin/master/testimonial/view",
    data: {
      id: id,
    },
    success: function (response) {
      if (response.err == 0) {
        ...
        $("#city").val(data.city);
        $("#testimonial_thumbnail_image").val(data.testimonial_thumbnail_image);
        $("#testimonial_video").val(data.video_url);
        $('#image_preview1').attr('src', data.testimonial_thumbnail_image).show();       
        $('#video_preview').attr('src', data.video_url).show();       
        const delta = quill1.clipboard.convert(data.testimonial_detail);
        quill1.setContents(delta);
        $(".popup-title").html("<h2>Edit Testimonial</h2>");
        $("#edit-modal").modal("show");
      }
    },
  });
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      jQuery.ajax({
        ...
        url: BASE_URL + "admin/master/testimonial/delete",
        data: {
          id: id,
        },
        success: function (response) {
...
function Reset() {
  ...
  $("#user_email").val("");
  $("#city").val("");
  $("#testimonial_thumbnail_image").val("");
