...
function pagination(start) {
    ...
    jQuery.ajax({
      type: "POST",
      url: `${BASE_URL}admin/master/features/list/${start}/${limit}`,
      headers: {
...
function html(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        htmlData += `<td>${list[i].features_title}</td>`; 
        htmlData += `<td>${list[i].features_description}</td>`;
...
function add_record(){;
    ...
    $("#edit_features").trigger("reset");
    $(".popup-title").html("<h2>Add Doctors Team</h2>");
}

const imageInput = $('#features_gif1');
    imageInput.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "features_gif"
...
$("#edit_features").on("submit", function (e) {
    e.preventDefault();
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          type: "POST",
          url: BASE_URL + "admin/master/features/add",
          headers: {
            authorization: token ? `Bearer ${token}` : "",
          },
          data: {
            id: $("#Id").val(),
            features_gif: $("#features_gif").val(),
            features_title: $("#features_title").val(),
            features_description: $("#features_description").val(),
  ...
  function view(id) { 
    ...
    jQuery.ajax({
      ...
      url: BASE_URL + "admin/master/features/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#Id").val(data._id);
          $("#features_gif").val(data.features_gif);
          $("#features_title").val(data.features_title);        
          $("#features_description").val(data.features_description);        
          $('#image_preview').attr('src', data.features_gif).show();
          $(".popup-title").html("<h2>Edit Features</h2>");
          $("#edit-modal").modal("show");
        }
      },
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          ...
          url: BASE_URL + "admin/master/features/delete",
          data: { 
            id: id,
          },
          success: function (response) {
