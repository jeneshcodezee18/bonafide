...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: `${BASE_URL}admin/master/international_patient/list/${start}/${limit}`,
    headers: {
...
function html(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    htmlData += '<td><img src="' + (list[i].international_patient_thumbnail_image) + '" alt="Image" width="50" height="50"></td>';
    // htmlData += "<td>" + new Date(list[i].createdAt).toLocaleDateString() + "</td>";
    htmlData += `<td>${(list[i].created_at).toLocaleString()}</td>`;
...
function add_record() {
  ...
  $("#edit_international_patient").trigger("reset");
  $(".popup-title").html("<h2>Add International Patient</h2>");
}

const imageInput1 = $('#international_patient_thumbnail_image1');
imageInput1.on('change', function (e) {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
        const file_field = "international_patient_thumbnail_image"
        handleFileUpload(selectedFile, file_field);
    }
});
const videoInput2 = $('#international_patient_video1');
videoInput2.on('change', function (e) {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
        const file_field = "international_patient_video"
        handleFileUpload(selectedFile, file_field);
    }
});
function handleFileUpload(selectedFile, file_field) {
  ...
  $.ajax({
      ...
      success: function (response) {
          ...
          if (file_field === 'international_patient_thumbnail_image') {
              $('#image_preview1').attr('src', imagePath).show();
          }else if (file_field === 'international_patient_video') {
            $('#video_preview').attr('src', imagePath).show();
        }
...
$("#edit_international_patient").on("submit", function (e) {
  e.preventDefault();
  ...
  Swal.fire({
    ...
    customClass: {
      ...
      if (result.isConfirmed) {
          ...
          jQuery.ajax({
        type: "POST",
        url: BASE_URL + "admin/master/international_patient/add",
        headers: {
          authorization: token ? `Bearer ${token}` : "",
        },
        data: {
          id: $("#Id").val(),
          international_patient_thumbnail_image: $("#international_patient_thumbnail_image").val(),
          international_patient_video: $("#international_patient_video").val(),
...
function view(id) {
  ...
  jQuery.ajax({
    ...
    url: BASE_URL + "admin/master/international_patient/view",
    data: {
      id: id,
    },
    success: function (response) {
      if (response.err == 0) {
        ...
        $("#Id").val(data._id);
        $("#international_patient_thumbnail_image").val(data.international_patient_thumbnail_image);
        $("#international_patient_video").val(data.video_url);
        $('#image_preview1').attr('src', data.international_patient_thumbnail_image).show();       
        $('#video_preview').attr('src', data.video_url).show();       
        $(".popup-title").html("<h2>Edit International Patient</h2>");
        $("#edit-modal").modal("show");
      }
    },
  });
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      jQuery.ajax({
        ...
        url: BASE_URL + "admin/master/international_patient/delete",
        data: {
          id: id,
        },
        success: function (response) {
...
function Reset() {
  $("#Id").val("");
  $("#international_patient_thumbnail_image").val("");
  $("#image_preview1").attr("src", "/assets/media/svg/avatars/blank.svg");
  $("#international_patient_video").val("");  
