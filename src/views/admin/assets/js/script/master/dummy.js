...
function pagination(start) {
    ...
    jQuery.ajax({
      type: "POST",
      url: `${BASE_URL}admin/master/dummy/list/${start}/${limit}`,
      headers: {
...
function add_record(){;
    ...
    $(".popup-title").html("<h2>Add Dummy</h2>");
}


$("#edit_dummy").on("submit", function (e) {
    e.preventDefault();
    // const editorContentHTML1 = editor1.getData();
    var editorContentHTML1 = editor1.getHTMLCode();
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          type: "POST",
          url: BASE_URL + "admin/master/dummy/add",
          headers: {
            authorization: token ? `Bearer ${token}` : "",
          },
          data: {
              id: $("#Id").val(),
              editor1: editorContentHTML1,
  ...
  function view(id) { 
    ...
    jQuery.ajax({
      ...
      url: BASE_URL + "admin/master/dummy/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#Id").val(data._id);
          editor1.setHTMLCode(data.editor1);
       
          $(".popup-title").html("<h2>Edit Dummy</h2>");
          $("#edit-modal").modal("show");
        }
      },
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          ...
          url: BASE_URL + "admin/master/dummy/delete",
          data: { 
            id: id,
          },
          success: function (response) {
