...
function pagination(start) {
    ...
    jQuery.ajax({
      type: "POST",
      url: `${BASE_URL}admin/master/faq/list/${start}/${limit}`,
      headers: {
...
function html(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        htmlData += `<td>${list[i].faq_question}</td>`; 
        htmlData += `<td>${list[i].faq_answer}</td>`;
        htmlData += "<td>" + new Date(list[i].createdAt).toLocaleString() + "</td>";
...
function add_record(){;
    ...
    $(".popup-title").html("<h2>Add FAQ</h2>");
}
...
$("#edit_faq").on("submit", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          type: "POST",
          url: BASE_URL + "admin/master/faq/add",
          headers: {
            authorization: token ? `Bearer ${token}` : "",
          },
          data: {
  ...
  function view(id) { 
    ...
    jQuery.ajax({
      ...
      url: BASE_URL + "admin/master/faq/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $(".popup-title").html("<h2>Edit FAQ</h2>");
          $("#edit-modal").modal("show");
        }
      },
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          ...
          url: BASE_URL + "admin/master/faq/delete",
          data: { 
            id: id,
          },
          success: function (response) {
