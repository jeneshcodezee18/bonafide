...
function pagination(start) {
    ...
    levelFilterId = $("#imageCategoryDropdown").val();
    const search = $("#search").val();
  
    paginationStart = ((start - 1) * limit) + 1
    jQuery.ajax({
      type: "POST",
      url: `${BASE_URL}admin/master/gallery/list/${start}/${limit}`,
      headers: {
  ...
  $("#imageCategoryDropdown").change(function (e) {
...
function html(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        htmlData += '<td><img src="' + (list[i].media_type == 1 ? list[i].image : list[i].gallery_thumbnail_image) + '" alt="Image" width="50" height="50"></td>';
        htmlData += `<td>${list[i].media_type == 1 ? 'img' : 'video'}</td>`;
        htmlData += `<td>${list[i].image_category ? list[i].image_category : 'video'}</td>`;
...
function add_record(){;
    ...
    $("#edit_gallery").trigger("reset");
    $(".popup-title").html("<h2>Add gallery</h2>");
}
...
const imageInput = $('#image1');
    ...
    const imageInput1 = $('#gallery_thumbnail_image1');
    imageInput1.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "gallery_thumbnail_image"
            handleFileUpload(selectedFile, file_field);
        }
    });
    const videoInput2 = $('#gallery_video1');
    videoInput2.on('change', function (e) {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            const file_field = "gallery_video"
            handleFileUpload(selectedFile, file_field);
        }
    });
    ...
    function handleFileUpload(selectedFile, file_field) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                }else if (file_field === 'gallery_thumbnail_image') {
                  $('#image_preview1').attr('src', imagePath).show();
              }else if (file_field === 'gallery_video') {
...
$("#edit_gallery").on("submit", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          type: "POST",
          url: BASE_URL + "admin/master/gallery/add",
          headers: {
            authorization: token ? `Bearer ${token}` : "",
          },
          data: {
              ...
              image: $("#image").val(),
              gallery_thumbnail_image: $("#gallery_thumbnail_image").val(),
              gallery_video: $("#gallery_video").val(),
            },
            success: function (response) {             
  ...
  function view(id) { 
    ...
    jQuery.ajax({
      ...
      headers: {
       ...
       url: BASE_URL + "admin/master/gallery/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#Id").val(data._id);
          $("#media_type").val(data.media_type);
          $("#image_category").val(data.image_category);
          $("#image").val(data.image);     
          $("#gallery_thumbnail_image").val(data.gallery_thumbnail_image);
          $("#gallery_video").val(data.gallery_video);   
          $(".popup-title").html("<h2>Edit gallery</h2>");
          $("#edit-modal").modal("show");
        }
      },
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          ...
          url: BASE_URL + "admin/master/gallery/delete",
          data: { 
            id: id,
          },
          success: function (response) {
