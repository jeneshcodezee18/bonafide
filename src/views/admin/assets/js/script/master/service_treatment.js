...
function pagination(start) {
    ...
    jQuery.ajax({
      type: "POST",
      url: `${BASE_URL}admin/master/service_treatment/list/${user_id}/${start}/${limit}`,
      headers: {
...
function html(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        htmlData += `<td>${list[i].heading}</td>`;  
        htmlData += `<td>${list[i].service_treatment_sort}</td>`;  
...
function add_record(){;
    ...
    const user_id = window.location.pathname.split('/')[4];
    $("#service_name_id").val(user_id);

}

const imageInput1 = $('#logo1');
...
$("#edit_service_treatment").on("submit", function (e) {
    e.preventDefault();
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({ 
          type: "POST",
          url: BASE_URL + "admin/master/service_treatment/add",
          headers: {
            authorization: token ? `Bearer ${token}` : "",
          },
          data: {
              id: $("#Id").val(),
              logo: $("#logo").val(),
              service_treatment_sort: $("#service_treatment_sort").val(),
              heading: $("#heading").val(),
              description: $("#description").val(),
              service_name_id: $("#service_name_id").val(),
  ...
  function view(id) { 
    ...
    jQuery.ajax({
      ...
      url: BASE_URL + "admin/master/service_treatment/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#Id").val(data._id);
          $("#service_treatment_sort").val(data.service_treatment_sort); 
          $("#logo").val(data.logo); 
          $('#image_preview1').attr('src', data.logo).show();       
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        jQuery.ajax({
          ...
          url: BASE_URL + "admin/master/service_treatment/delete",
          data: { 
            id: id,
          },
          success: function (response) {
