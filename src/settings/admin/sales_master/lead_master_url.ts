...
export function bindURL(): void {
  app.get(
    "/admin_master/report/lead_master",
    requireAuth,
  ...
  app.get(
    ...
    async function (req: Request, res: Response): Promise<void> {
      try {
        ...
        res.render(
          "admin/sales_master/lead_master/add_lead_master.html",
          respData
        );
      } catch (err: unknown) {
        console.error(err);
  ...
  app.post(
    "/admin_master/lead_master/delete",
    async function (req: Request, res: Response): Promise<void> {
      try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
  ...
  app.post(
    "/admin_master/lead_master/list/:start/:limit",
    async function (req: Request, res: Response): Promise<void> {
      try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
  ...
  app.post(
    "/admin_master/assigned_members/list",
    async function (req: Request, res: Response): Promise<void> {
      try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
          ...
          apiJwtController.DECODE(req, (respData: any) => {
            if (respData.status !== 200) {
              ...
              data.start = start;
              data.limit = limit;
              salesMasterController.LIST_ASSIGNED_MEMBERS(
                data,
                function (respData) {
                  res.status(respData.status).send(respData);
                }
